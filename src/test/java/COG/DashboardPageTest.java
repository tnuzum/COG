...
public class DashboardPageTest extends base{
	...
	@Test (priority = 1, description="Header Verifications")
		public void headerVerifications()
		{
			...
			Assert.assertEquals(baseURL+"CompeteOnTheGo/Account/ChangePassword", d.getChangePasswordLink().getAttribute("href"));
	...
	@Test (priority = 2, description="Front Desk Tile Verifications")
		public void frontDeskTileVerifications()
		{
			...
			Assert.assertEquals(baseURL+"CompeteOnTheGo/FrontDesk/PlanBuilder/Enrollment", d.getFDeskMemberGuestEnroll().getAttribute("href"));
			...
			Assert.assertEquals(baseURL+"CompeteOnTheGo/Frontdesk/PointOfSale", d.getFDeskPOS().getAttribute("href"));//note lowercase 'd' in Frontdesk
			Assert.assertTrue(d.getFDeskPOS().isEnabled());
			
			Assert.assertEquals("Class Check-In", d.getFDeskClassCheckIn().getText());
			Assert.assertEquals(baseURL+"CompeteOnTheGo/Frontdesk/ClassCheckIn", d.getFDeskClassCheckIn().getAttribute("href"));//note lowercase 'd' in Frontdesk
	...
	@Test (priority = 3, description="Back Office Tile Verifications")
		public void backOfficeTileVerifications()
		{
			...
			Assert.assertEquals(baseURL+"CompeteOnTheGo/Backoffice/Home", d.getBOfficeHomeLink().getAttribute("href"));//note lowercase 'o' in Backoffice
			Assert.assertTrue(d.getBOfficeHomeLink().isEnabled());
			
			Assert.assertEquals("Member Management", d.getBOfficeMemberMgmt().getText());
			Assert.assertEquals(baseURL+"CompeteOnTheGo/BackOffice/MemberManagement", d.getBOfficeMemberMgmt().getAttribute("href"));
			Assert.assertTrue(d.getBOfficeMemberMgmt().isEnabled());
			
			Assert.assertEquals("Club Taxes", d.getBOfficeClubTaxes().getText());
			Assert.assertEquals(baseURL+"CompeteOnTheGo/BackOffice/ClubTaxes", d.getBOfficeClubTaxes().getAttribute("href"));
			...
			Assert.assertEquals(baseURL+"CompeteOnTheGo/BackOffice/AgreementManagement", d.getBOfficeAgreementMgmt().getAttribute("href"));
